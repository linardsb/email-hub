...
- **container-fit:** ``ghost <table width> + Σ(host horizontal paddings) ==
  container`` (600, or 640 for c6/c9), with an ``align="center"`` underflow
  exception (a centered strip whose total < container is valid — assert only
  self-consistency). Above container ``>`` is always a FAIL (the F7 overflow
  symptom).
...
the gate can never drift from what the converter actually emits.
...
class _GhostWalker(HTMLParser):
    ...
    def handle_comment(self, data: str) -> None:
        ...
        elif sep:  # column separator -> nearest percol frame
            frame = self._top_percol()
            if frame is not None:
                frame["tds"].append(int(sep.group(1)))
        elif skeleton_close and self.frames:  # CLOSE -> pop most-recent frame
...
class TestWordBoxArithmetic:
    ...
    def test_gate_catches_overflow(self) -> None:
        ...
        over = (
            _load("7")
            .replace('width="512"', 'width="560"')
            .replace('width="256" valign="top"', 'width="280" valign="top"')
        )
