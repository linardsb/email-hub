...
### Track C — Segmentation spike (C1, C2) `[Backend]` `[✅ Spike done 2026-06-11 · C1+C2a shipped gated (NOT committed); C2b rejected]`
...
**Implementation (as built — diverged from the plan sketch):**
- `[✅ Done]` **C1** — `group_by_wrapper()` (`app/design_sync/sibling_detector.py`) regroups sections by their exact `parent_wrapper_id` (stamped by the unwrap pre-pass), wired into `_match_phase` behind `DESIGN_SYNC__BAND_GROUPING_ENABLED` (default OFF). Grouping done at **match phase**, not in `_expand_container_wrappers` as sketched — the band tags already exist, only the renderer needed to honour them. Also fixed the real count bug: `sections_count` `len(match.matches)` (flat) → `len(grouped_sections)` (bands). _Article-card `_inner` seed-steering not done — separate seed-match concern, deferred._
- `[✅ Done] C2a` — SPACER absorption inside `group_by_wrapper(absorb_spacers=True)`.
- `[✗ Rejected] C2b` — single-child wrapper recursion. Naive deep recursion over-segments (arity-based `_is_section_child` can't find boundaries deep in the tree); the constrained one-level peel fixed maap 9→13 + starbucks 5→9 **but regressed perf 10→13**. `physical_card_detector.find_physical_card_in_subtree` is card-specific, not a section walker — not reused.
**Result (working tree, NOT committed per "do NOT full-commit"):** ✅ **C1+C2a fixes the over-segmenters EXACTLY** — LEGO 17→8, slate 10→8, perf 11→10 — HTML-verified (real band tables, correct wrapper fills, spacing intact). Flag-OFF byte-identical to main; mypy/pyright/lint clean; 1891 design_sync tests + golden-conformance green; 7 new `group_by_wrapper` unit tests. **Definitive finding for 53.1:** the under-count is **semantic, not structural** — `mj-section → N mj-column` is 1 section (perf stats) or N (starbucks cards) by *content meaning*, identical structure + geometry; no deterministic rule separates them. This is the genuine fork-(b)/VLM signal (was suspected, now proven). Under-segmenters (maap/starbucks/mammut) remain the fork decision input. Full write-up: `docs/phase-53-track-c-spike.md`.
