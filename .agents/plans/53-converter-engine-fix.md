...
**Status (2026-06-12): ✅ A4 COMPLETE — path (a) executed; all 6 fixtures resolve (verify criterion was ≥2).** Tooling: `scripts/export-case-assets.py` (shipped in PR #241 `7894ff03`) reads emitted node-ids from each case's `expected.html` and renders them via the Figma image API (file `VUlWjZGAEVZr3mK1EawsYR`, scale=2 — verified byte-identical to the import pipeline on case 5); bookkeeping in `.agents/figma-cache/exports.json`. Exports ran locally: cases 6/10 assets dated Apr 24, cases 7/8/9 dated Jun 3. **Independently re-verified 2026-06-12** via fresh `run_case_conversion()` per case: every emitted node-id has an on-disk `assets/<id : → _>.png` for all of cases 5–10 — `missing=0` everywhere, including **LEGO (case 7, the over-segmenter: 22/22)**. Surplus PNGs (case 5: 92, case 6: 21, case 10: 34) are from broader earlier exports — harmless. Caveats: (1) assets are **gitignored** (`data/debug/*/*`, line 114) → local-only; CI cannot run the pixel metric, and a fresh clone must re-run the script with a `FIGMA_TOKEN` (same variance class §A1 warns about — surfaced, not resolved here); (2) LEGO's reference image is `…/for_converter_engine/Lego/viaual_design.png` (known typo, deliberately preserved — see `50-current-output-reference.md`); account for it when wiring LEGO into the §A3 scorer. **§A3 is now unblocked for multi-fixture (≥2 incl. over-segmenter) wiring.**
...
**Status (2026-06-10):** ✅ B1 · B5 · B7 · B8 shipped (PR #241, squash `7894ff03`). ✅ **B2** (commit `c3f7fcd8`) — `_build_column_fill_html` + round-robin `_build_column_fills` now wrap img/text/CTA rows in one inner `<table role="presentation">` via shared `_column_image_row`/`_column_cta_row`/`_wrap_column_table` helpers; baselines 5–10 regenerated. ✅ **B3** `925d4e05` (blank unfilled text slots), **B4** `0bf96eda` (de-truncate nested `footer_content`), **B6** `02bb1b8e` (clamp `width="640"`/`max-width:640px` MSO widths to container — latent on corpus, unit-tested). ✅ **B8 follow-up** `b50dcf13` — per-button cta-pair color fidelity (closed `phase-53-b8-cta-pair-color-fidelity`). **Track B (B1–B8) COMPLETE — merged to `main` via `--no-ff` merge `f9787f2c` (2026-06-10).** Residual deferred-items as separate follow-ups: `phase-53-b5-decorative-empty-alt-vs-g3neg`, `phase-53-b8-{non-cta-multibutton-drop,fills-cta-slug-desync-vlm}`.
...
|---|---|---|---|
| Closes segmentation (5/6) | **Yes** — Track C is buildable (renderer nests, §2.1); risk = heterogeneous bands | ~~**Yes by construction**~~ **REFUTED on fixtures** (53.1 gate: candidates row 9/5/12 vs targets 13/9/18 — a faithful tree walk reproduces the under-count; see `.agents/plans/53-1-fork-decision.md` §finding 2) | Yes (whole frame as image) |
...
**Status (2026-06-12): fork (a) executed per `.agents/plans/53-d-fork-a-execution.md` — D1/D2 SHIPPED, D3 spiked (gate pending).**

- **D1 (`50c691b2`):** `band_grouping_enabled` default **ON** (env var = kill switch, cull by 2026-09-10); baselines 5/7/8/9/10 regenerated after intended-vs-structural diff audit (zero content loss); ladder snapshot regenerated — rendered == bands on all 6. **A2 target gate strict for 7/8/9 in BOTH suites** (data-regression + snapshot), 5/6/10 xfail keyed on `SEMANTIC_UNDERCOUNT_CASES` (ladder_harness.py). Deferred `phase-53-a2-advisory-section-gate` partially closed (over-segmenter half).
- **D2 / A8 (`e20f7ad7`):** `compute_column_width_fractions` (analyzer) → `EmailSection.column_width_fractions` → renderer rewrites the column seed's MSO ghost-`<td>` + div `max-width` together (5% equal-tolerance fallback). Required a **document-bridge fix**: `DocumentSection.to_email_section` recomputes fractions from round-tripped column widths — without it the field died at the bridge (RC-A inert-bridge class). Corpus byte-stable except case 5 (width attrs only: product grids now 152/157/291 + 148/156/118/178). Ladder counts unchanged; A3 Δ −0.0004.
- **D3 spike (`f632b8df`, default OFF):** constrained one-level peel + content-scale discriminator (`_grandkids_are_cards`: imagery OR ≥48px column height → peel; short image-free cells → keep) behind `DESIGN_SYNC__SEMANTIC_PEEL_ENABLED`. **Heuristic alone hits both reachable targets: maap 9→13 EXACT, starbucks 5→9 EXACT, 7/8/9 untouched (8/10/8).** Mammut 12→12 — its gap is **below candidate level** (candidates=12 vs target 18; not the single-child-wrapper shape) → 53.7 ceiling doc. A3 flag-on: starbucks **+0.018**, maap **−0.026** full-image (peeled side-by-side cards render stacked; section_median rises 0.870→0.917). **Ship/park = user gate on these numbers**; VLM variant not needed for the count targets (sub-plan §D3.4).
