...
- [x] **Part D3** — superseded by `.agents/plans/tech-debt-08c-converter-deletion.md` (executed 2026-05-06). The 14-day calendar window was retired early when (a) static analysis confirmed zero non-test, non-self callers and (b) the readiness grep across all `traces/*.jsonl` returned zero hits. 08c delivered: helper extraction (part 1, `6b136968`), test migration (part 2, `9a4e3b41`), shim deletion (part 3, `e990a9c9`), and full `converter.py` removal + `_convert_recursive` deletion + legacy renderer test purge (part 4, `113e1114`).
- [x] **Mark F010, F011, F012, F013 as RESOLVED in `TECH_DEBT_AUDIT.md`** — F010+F013 closed in Plan 08c part 4 (`113e1114`); F011 in Plan 08 Part B (`bf66bd0a`); F012 in Plan 08 Part C + 08b.
