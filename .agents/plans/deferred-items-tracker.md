...
| 3 | ~~**C — Tenant-isolation harness**~~ | ~~`tech-debt-03-tenant-isolation-harness.md`~~ | ~~soft~~ | ~~New repo ships without `scoped_access` filter; no integration net catches it~~ | ~~`tech-debt/03-tenant-iso-harness`~~ | ~~`make test-integration` runs `test_tenant_isolation.py` green w/o `TEST_DATABASE__URL` gate; 4 xfail entries promoted~~ — **closed 2026-05-10** via `tech-debt/03-tenant-iso-harness`: `app/tests/conftest.py` provides session-scoped `_integration_engine` (alembic-upgrade-head) + per-test `db` (`TRUNCATE … RESTART IDENTITY CASCADE`); `test_tenant_isolation.py` wires all 5 entities via `EntitySpec`; `xfail(strict=True)` dropped from templates/memory/qa_results/approvals; briefs follow-up tracked as `tech-debt-03-briefs-user-isolation-test`. |
| 4 | ~~**D — Design-sync 50.7 cluster + stranded templates**~~ | ~~`phase-50.8-lego-promotion-and-detector-followup.md`~~ | ~~soft + speculative (4 entries)~~ | ~~Phase 52.7 dark-mode flip mis-inverts physical cards; Phase 51A regressions ship invisible~~ | ~~`phase/50.8-lego-promotion`~~ | ~~LEGO + performance_reimagined + slate in `data/debug/`; `make converter-data-regression` covers them; gap-2/gap-3 empirically closed or moved to confirmed-bug status.~~ — **closed 2026-05-12** via `phase/50.8-lego-promotion`: 3 fixtures landed (data/debug/7/8/9); converter-data-regression climbed 38 passed → 65 passed; AC #4 closed via new `TestLEGORealFixture` against real fixture; gap-2 refuted; gap-3 superseded by new `phase-50.8-nested-physical-cards` (layout_analyzer needs subtree-walking — Phase 52.7 prereq). |
| 5 | ~~**E — Pyright `reportDeprecated` demotion**~~ | ~~`tech-debt-pyright-deprecation-restore.md`~~ | ~~speculative~~ | ~~Real Pydantic v3 / SQLAlchemy 2.x deprecations land as warnings, slip CI~~ | ~~`tech-debt/pyright-restore-deprecated`~~ | ~~`reportDeprecated = "error"` restored OR upstream fix documented in code comment with version pin~~ — **closed 2026-05-13** via `tech-debt/pyright-restore-deprecated`: migrated 5 `@(async)contextmanager` sites (`database.py:42`, `scoped_db.py:131`, `scoped_db.py:153`, `main.py:72`, `test_seed_graph.py:37`) from `(Async)Iterator[T]` to `(Async)Generator[T, None]` / `Generator[T, None, None]`; `reportDeprecated = "warning"` demotion dropped from `pyproject.toml:323-325`; `make types` green at 0 errors. |
