...
| 2 | **B — Alembic schema drift** | `tech-debt-19-alembic-schema-drift.md` | known-bug | Future `--autogenerate` run produces destructive 200-op migration | `tech-debt/19-alembic-drift` | `alembic check` exits 0 against fresh DB; CI step drops `continue-on-error: true` |
