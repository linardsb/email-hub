...
**Result (2026-07-05, `fix/phase-53f-f7-pills-radius`) — LEGO-card half DONE (renderer-only), plan
`.agents/plans/53-f7-column-card-surface.md`:** `_wrap_col_bg_inner_card` (`component_renderer.py`)
mirrors the RC-F2 insert-on-no-op — when the `_inner` bg override finds no target AND a `col[234]-bg`
cell exists, it wraps that cell's body (via `_find_matching_close`) in ONE
`<table class="product-card _inner">`. Reuses the existing `_inner` bg/radius overrides + the static
`product-card` dark class (`converter_service.py:1083` → `#2d2d44`); **`_detect_inner_bg` /
physical-card detection untouched** (item-2 answer confirmed correct). Seeds + `component_matcher.py`
**not touched** (byte-safe insert beats the seed-wrapper sketch — the divergence held up). **c7
0.636→0.719 (+0.083)**, section_median → 0.804; **only c7's expected.html changed** (git-confirmed),
**exactly 6 wraps** (sec 5/7/9/11/13/15), c5/c6/c8/c9/c10 byte-identical (snapshot regression 34
passed / 1 xfail; ladder 13/9/8/10/8/12 held). c7 cards are rounded (`inner_radius=12`, not square as
the plan assumed) — one bug found+fixed in execution: the wrapper base must NOT set
`border-collapse:collapse` or it overrides the radius path's prepended `separate` and defeats corner
clipping. 10 unit tests (`test_column_card_surface.py`, 6 RED pre-fix, real seeds w/ MSO ghosts).
Visual (composite): uniform white rounded cards on lime, not ragged. Membership card [19]
(`image-gallery`, carries `inner_bg` but a different seed) still surface-less → new deferred
`phase-53f-f7-image-gallery-membership-card`; lime-float + per-corner rounding stay 51.1.
...
| 4 | Column detection widening (bare IMAGE+TEXT columns; peel beyond `mj-wrapper` naming) | ⏳ new plan | Only if post-F corpus still shows stacked 2-cols; needs non-MJML fixtures first (53.6 stub). **Close-out finding (2026-07-05): the condition FIRED but the mechanism is different** — stacked 2-cols persist corpus-wide, yet detection is correct (sections match `column-layout-{2,4}`); the stacking is a RENDER width-budget bug (§6 close-out, c7 item 1). A detection-widening plan would aim at the wrong lever; the needed slice is renderer-side width rescale (F9-class), no non-MJML fixture dependency. |
...
| 2026-07-05 | F7 (cards) | 0.845 | 0.802 | **0.719** | 0.802 | 0.679 | 0.678 | **Pixel win (LEGO white cards), renderer-only.** Plan `.agents/plans/53-f7-column-card-surface.md`. `_wrap_col_bg_inner_card` (`component_renderer.py`) inserts ONE `<table class="product-card _inner">` around the `col[234]-bg` cell body when the `_inner` bg override no-ops (RC-F2 insert-on-no-op pattern) — reuses existing `_inner` overrides + static `product-card` dark class (`#2d2d44`), zero `_detect_inner_bg` change. **c7 0.636→0.719 (+0.083)**, section_median → **0.804** (maap/starbucks band), section_min 0.351 (still capped by the F1 blank hero + out-of-scope membership card). **Only c7 changed** (git-confirmed single-file baseline diff; snapshot regression 34 passed/1 xfail): **exactly 6 wraps** (sec 5/7/9/11/13/15 `column-layout-2`); c7's 7th col-bg + sec[19] `image-gallery` membership card byte-identical within c7; c5/c6/c8/c9/c10 **byte-identical**. Cards render rounded (`inner_radius=12` → `border-radius:12px`, `border-collapse:separate;overflow:hidden` clips — plan's "square card" assumption was wrong). Visual: uniform white rounded cards on lime, not ragged (per-section confirmed). Ladder 13/9/8/10/8/12 held; 10 unit tests (`test_column_card_surface.py`, 6 RED pre-fix). Closes `phase-53f-f7-column-seed-no-inner`; membership card → new deferred `phase-53f-f7-image-gallery-membership-card`. **Outlook latent (logged, non-blocking):** the section's `_cell` 20px override now lands on the wrapper cell, which sits OUTSIDE the 600px MSO ghost → card ~40px too wide in the Word engine (Chromium-correct + unscored; no gate renders Outlook, cf. F3's attr/width latent) → deferred `phase-53f-f7-card-wrapper-outlook-ghost-overflow`, resolved by the 51.1 card-padding restructure (threads padding inside the per-column ghost cells). |
...
| 2026-07-05 | **Track F close-out (final)** | 0.845 | 0.802 | 0.719 | 0.802 | 0.679 | 0.678 | Final full_image (fresh run, F7-cards `97bfb77c` in tree); section_min 0.492/0.477/0.351/0.688/0.353/0.087; section_median 0.821/0.699/0.804/0.868/0.745/0.777. All 6 composites eyeballed per §3.3 — verdict below. |
| 2026-07-06 | assets restored (ops, no code) | 0.845 | 0.802 | **0.770** | 0.782 | 0.680 | 0.678 | **Fixture repair, not code — zero source/baseline changes** (`expected.html` md5-identical on all 6; `git status data/debug` clean). `scripts/export-case-assets.py 5..10` with the **live connection-5 DB token** (`.env DESIGN_SYNC__FIGMA_PAT` is expired — Figma `/v1/me` says "Token expired"; the DB token last4 `3V5R`, refreshed 2026-07-05, works). All 4 gap nodes on disk: c7 hero `2833:1881` (798KB), c8 hero `2833:2264` (2.1MB), c9 col-icons `2833:2113`/`2833:2126` (1.2KB/320B — tiny-flagged, composite-verified real). **c7 +0.051** (blank hero → real Stranger-Things render; median 0.804→0.816). **c8 −0.020 is the F3-class height-band artifact** (real 400px hero inflates render height vs blank gap; median 0.868→0.788, section_min 0.688 flat — composite shows the hero correct). c9 +0.001 (section_min 0.353→**0.361** — pin/thermometer/clipboard/arrow render as real glyphs, broken-image boxes gone). c5/c6/c10 flat; **case-5 re-export byte-identical to committed PNGs** (§A1 same-input variance: none at scale=2 today). c10 re-export stalled in Figma 429 backoff — June asset set already complete, no gap. Local-only → `phase-53.7-asset-reexport-prerequisite` **stays open** (assets gitignored, no CI token). App-side ops-caveat validation (imports 39–41): see ledger `phase-53f-app-snapshot-serializer-drops-render-fields` + `phase-53f-document-path-ignores-node-selection` — re-sync + re-run does **not** restore CTA stroke/radius app-side. |
| 2026-07-06 | app-parity fixes A–D + text-block radius | 0.845 | 0.802 | **0.771** | 0.782 | 0.680 | 0.678 | **Closes the app-vs-harness split** (`fix/phase-53f-app-render-field-parity`): (A) snapshot `_file_structure` serializer round-trips ALL `DesignNode` render fields (`_serialization.py`; dataclass-parity test in `test_serialization_roundtrip.py`); (B) `run_conversion` document path only for whole-file imports — selected nodes always take the legacy filtered path (RED-proven both ways in `test_import_service.py`); (C) `_fills_text_block` threads `btn.border_radius` (fallback 4, mirrors `_column_cta_row`) — closes `phase-53f-f7-text-block-cta-hardcoded-radius`; baselines c5/c6/c7 regen, diff-audited **radius-only 3 lines** ('Discover →'/'Order your fall favorite'/'Explore now' 4→25px), c8 ws-churn reverted, c9/c10 byte-identical, ladder **13/9/8/10/8/12 held**; (D) `_fix_text_contrast` treats `a` as a background carrier + ranges start at the opening tag (own inline bg governs own label; 2 RED-proven tests) — kills the white-on-white repaint. **c7 0.770→0.771** (r25 pill), c5/c6 flat (sub-rounding). **End-to-end (the §3 ops-caveat validation, now green): re-sync → snapshot 21 (stroke ×277/radius ×631/text_align ×2615) → import 43 → tmpl 46 renders 'Explore now' `#FFFFFF` bg / `#000000` text / `border:2px solid #000000` / `border-radius:25px` + centered headings — harness parity through the API, screenshot-verified.** `make check-full` green. Ledger: 3 entries closed (serializer, document-path, text-block radius); `phase-53-b8-…-text-color` (solid-branch) stays open. |
| 2026-07-06 | **F9 column width budget** | 0.840 | 0.802 | **0.794** | 0.778 | 0.680 | **0.742** | **Pixel win (stacked columns un-stack), renderer-only.** Plan `.agents/plans/53-f9-column-width-budget.md`; closes `phase-53f-column-width-budget` (close-out c7 item 1, the biggest remaining lever). `_shrink_column_ghost_widths` (+`_card_wrapper_inset`, `_style_horizontal_padding_px`, `component_renderer.py`) rescales the column seeds' three width surfaces TOGETHER (ghost `td width`, div `max-width`, ghost `<table width>` total) to the box the renderer's own insets leave: `render_section` 3c subtracts the RC-F7 card wrapper's relocated `_cell` padding; `render_repeating_group` subtracts `2×item_spacing.horizontal` per member row. A8 fractions preserved (`_distribute_widths` on current fractions), all-or-nothing invariant kept (count mismatch ⇒ whole no-op), slug-gated like A8. **15 sections rescaled corpus-wide** (diff-audited width-only, 162 lines): c7 6 carded benefit cards 600→**512** + "Andy\|0" col4 600→552; c8 2 spec grids →552; c10 5 product grids →552; c5 2 A8-asymmetric col3 pill rows →552 (fractions held); c9 1 col2 →552 (also heals its pre-existing 640-table/600-cells mismatch — 640-container design, live box at the 600 scoring viewport is 552; `total−inset` deliberately chosen over `table−inset`, which would re-wrap at 600). **c6 byte-identical; c8 sec[9] un-inset col4 control untouched.** Composite-verified: **c7 cards image-LEFT/text-RIGHT side-by-side, render 3144px vs 3223px design (was ~4470 app export); c7 sec[13] 4 cells one line; c8 spec grid 2×2; c10 product grid 2-up; c5 pills 3-per-line.** **c7 +0.023 (section_min 0.351→0.478), c10 +0.064 (median 0.777→**0.821**)**; c5 −0.005 / c8 −0.004 are the F3-class height-band artifact (un-stacking shortens renders toward design height; c8 median UP 0.788→0.791, c5 min sits on the pre-existing blank KASK product-image band); c9 flat to 3dp (its col2 icon row now side-by-side but ~60px on a 4242px height-normalized canvas). Ladder **13/9/8/10/8/12 held** (73 passed/1 xfail); 21 unit tests (`test_column_width_budget.py`, 5 RED pre-fix via public render paths). **Outlook note (not claimed):** rescaled ghost totals make the padded Word box fit arithmetically (512+88=600) — `phase-53f-f7-card-wrapper-outlook-ghost-overflow` STAYS OPEN pending Outlook verification. |

| 2026-07-06 | **F10 column y-order** | 0.840 | 0.802 | 0.794 | 0.778 | 0.680 | 0.742 | **Correctness win, not pixels (close-out c7 item 5).** Plan `.agents/plans/53-f10-column-y-order.md`; closes `phase-53f-column-category-order`. `_build_column_fill_html` now emits column rows in design tree order via `ColumnGroup.content_order` (node ids captured pre-order at both group constructors, `layout_analyzer.py`) + a stable merge (`_ordered_column_elements`, `component_matcher.py`); round-tripped through `DocumentColumn` (`email_design_document.py`, the #327 bridge lesson); empty order (older persisted documents, the content-group conversion) ⇒ legacy bucket order byte-for-byte. **full_image flat on all 6; only c7's baseline changed** — pure row relocation in exactly the 6 card sections (84 diff lines, zero style/content changes): pills 'Art prints'/'Stationery' move above their headings, 4 treats-card product names move above the pieces/points icon rows (icons interleave icon→label), CTAs stay last — **composite-verified before/after per §3.3**. c7 per-section: five card bands UP (+0.062/+0.052/+0.033/+0.030/+0.007), one −0.010 band happened to BE the median ⇒ section_median prints 0.803→0.793 while the mean is up (order-statistic optics; section_min 0.478 flat). c8 regen churn = pre-existing trailing-ws → skipped (F7-radius precedent); c5/c6/c9/c10 byte-identical (their mixed-order columns all sit in `layout=single` sections routed to slot-shaped seed builders — 51.1 territory, noted in plan §4, no new ledger entry). Ladder **13/9/8/10/8/12 held**; types 0-err; design_sync+components 2852/2xf; golden 26; 6 unit tests (`test_column_y_order.py`, 3 behavioral RED pre-fix via public paths). Pill radius stays 4px (no fixture radius; per-corner pill styling = 51.1). `phase-53-b8-text-block-solid-cta-text-color` stays open (F11 scope). |
| 2026-07-06 | **F11 text-block CTA label colour** | 0.840 | 0.802 | 0.794 | 0.778 | 0.680 | 0.742 | **Correctness win, not pixels — closes `phase-53-b8-text-block-solid-cta-text-color` (the b8 solid-branch residue; radius half shipped in #327).** The stroke-less branch of `_fills_text_block`'s CTA append hardcoded `color:#ffffff`; it now threads `_safe_color(btn.text_color, "#ffffff")` — white only as absence-fallback, exactly `_column_cta_row:842`. No shared-builder extraction: it would swap the outlined path's border/radius declaration order + stroke-weight formatting (`:.0f` vs `max(1,round)`) and churn c7/c10 baselines for zero visual gain; own-row CTA emission likewise skipped → 51.1 note. **b8's open question adjudicated from raw data, no render/VLM needed:** c7 `mj-button` `2833:1890` (fill #FFFFFF, stroke #000000/2.0 INSIDE, r25) + label TEXT `2833:1891` fill **#000000** prove extracted button-text fills are visual intent, not Figma defaults; c5 'Discover →' TEXT `2833:1640` fill **#000000** (the ledger's #010101 was the pre-re-audit fixture value); c6 `2833:1439` fill #FFFFFF — **white IS the c6 design**, so the predicted c6 label regen is a data-confirmed no-op. Diff-audited baselines: **c5 1 line (`color:#ffffff`→`#000000` on 'Discover →'; radius stays 25px)**; c6 1 case-only line (`#ffffff`→`#FFFFFF` — the extracted value's case now flows through, byte-parity with the column path, zero visual); c8 2 case-only 'DISCOVER IT' lines — a reverted c8 fails the snapshot gate's content compare once label bytes change (F10's skip precedent only held because its churn was ws-only), and **the regen's 3 trailing-ws churn lines never reach the repo: the pre-commit trailing-whitespace hook strips them at commit (the churn's actual origin — the converter emits trailing ws in c8 headings, the hook strips it from every committed baseline, the snapshot gate ws-normalizes → both sides stay green)**; c7/c9/c10 byte-identical (c7 outlined + white-labelled pills; c9's dark-label 'WATCH THE VIDEO' never renders — pre-existing template-path drop, out of scope; c10's CTA pair is outlined → already #010101). White-on-white grep `background-color:#FFFFFF;color:#ffffff` + any-case pair: **zero on all 6**. Scores flat to 3dp on all 18 metrics (a 152px label flip is sub-resolution for the band scorer). Ladder **13/9/8/10/8/12 held** (snapshot 34/10sk/1xf); types 0-err; design_sync+components 2853+; golden 26; 2 unit tests (`TestTextBlockCTALabelColor`, `test_cta_fidelity.py`): dark-label-on-light-fill **RED pre-fix** (failure output = the exact `background-color:#FFFFFF;color:#ffffff` invisible signature); white-fallback-when-text_color-absent is green pre-fix by construction (the hardcode coincides with the fallback) — it pins the fix against drifting to `_safe_color`'s #333333 default. |

### Close-out verdict vs §3 (2026-07-05)

**Band target (c7/c8/c9 full_image ≥0.80): 1 of 3 met.** c8 0.702→**0.802 MET**. c7 0.624→**0.719 MISSED** (+0.095; section_median 0.804 *is* in the maap/starbucks band — the full_image miss is height distortion from stacked columns + the blank hero, see below). c9 0.640→**0.679 MISSED** (+0.039; peaked 0.732 post-F2, gave the gain back to fixture asset-gaps + the F3 height artifact). **"No case regresses":** c5 0.879→0.845 is the F5 compliance trade (−0.033, accepted + documented at ship) plus pills (+0.001); c6 +0.001 / c10 −0.001 noise. No unexplained regression.

**Remaining mechanisms (composite-verified this session — honest per contract, no massage):**
- **c7 (0.719):** (1) **Column width budget — NEW RC, biggest remaining lever, previously unnamed.** Column seeds hardcode 600px-context pixel widths (`column-layout-2` ghost `td width="300"` + div `max-width:300px`; ×4 = 150s), but the live content box shrinks to ~512px under the band's 24px `_cell` inset + F7's 20px card padding → the inline-block columns **wrap: every 2-col card renders stacked** (an app-side export renders 4470px vs the 3223px design). Detection is CORRECT (sections match `column-layout-2`; §4 row 4's "column-detection widening" is the wrong lever); `_apply_column_width_fractions` (A8) redistributes but never rescales the seed total to the effective box. Renderer-side rescale = F9-class slice. (2) F1 blank-hero asset gap (`2833:1881` PNG unexported — fixture artifact, production unaffected). (3) Membership card [19] surface → `phase-53f-f7-image-gallery-membership-card` (51.1). (4) Text-block CTA path → `phase-53f-f7-text-block-cta-hardcoded-radius` + `phase-53-b8-text-block-solid-cta-text-color` ('Explore now' 4px; invisible-on-white risk when stroke data is absent). (5) **In-column category ordering:** `_build_column_fill_html` emits images→texts→buttons, discarding design y-order — the tag pill renders below the body (design: eyebrow above heading; F6 lifts *texts* only, a pill classifies as a button) and card icons render above product names.
- **c9 (0.679):** fixture asset gaps dominate (F4b col-icon `/api` PNGs + F1-class heroes absent → broken-image boxes on white bands; production serves these — `phase-53.7-asset-reexport-prerequisite`); article-card fills 2/5 ("HOT TRIP, COOL GEAR." heading + CTA absent from render); F3's height artifact vs the card-inflated reference persists (aspect 2.63 vs 3.31).
- **c10 (0.678):** under-segmentation (A2 12 vs 18, mammut xfail — count work out of scope per §3.2) + ingest-inflated nav arrows (28→268px, `phase-53f-decorative-image-flag`) + product grid rendering single-column (same width/grid class as c7 item 1).
- **Ops note (not an engine gap):** an app-side LEGO run ingested 2026-04-03 renders far below the harness on identical renderer code — its DB-stored structure predates the radius/stroke/text-color/alignment extraction fields (invisible outlined CTA, 4px pills, left-aligned headings). Stale design runs need a design-sync re-run; the frozen `data/debug` fixtures are unaffected.
